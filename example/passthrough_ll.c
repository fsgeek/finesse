...
static const struct fuse_opt lo_opts[] = {{"writeback", offsetof(struct lo_data, writeback), 1},
...
static void lo_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    if (lo_debug(req))
        fuse_log(FUSE_LOG_DEBUG, "lo_lookup(parent=%" PRIu64 ", name=%s, length=%zu)\n", parent, name, strlen(name));
...
static void lo_rename(fuse_req_t req, fuse_ino_t parent, const char *name, fuse_ino_t newparent, const char *newname,
                      unsigned int flags)
{
    int res;
...
static void lo_setxattr(fuse_req_t req, fuse_ino_t ino, const char *name, const char *value, size_t size, int flags)
{
    char             procname[64];
    struct lo_inode *inode = lo_inode(req, ino);
    ssize_t          ret;
...
static void lo_copy_file_range(fuse_req_t req, fuse_ino_t ino_in, off_t off_in, struct fuse_file_info *fi_in, fuse_ino_t ino_out,
