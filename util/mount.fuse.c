...
static int prepare_fuse_fd(const char *mountpoint, const char *subtype, const char *options)
{
    int   fuse_fd      = -1;
    int   flags        = -1;
...
static void drop_and_lock_capabilities(void)
{
    ...
    if (prctl(PR_SET_SECUREBITS, SECBIT_KEEP_CAPS_LOCKED | SECBIT_NO_SETUID_FIXUP | SECBIT_NO_SETUID_FIXUP_LOCKED | SECBIT_NOROOT |
    ...
    for (cap = 0;; cap++) {
        ...
        if (cap_status != 1) {
            fprintf(stderr, "%s: Failed to get capability %u: %s\n", progname, cap, strerror(errno));
            exit(1);
        }
        if (prctl(PR_CAPBSET_DROP, cap) == -1) {
            fprintf(stderr, "%s: Failed to drop capability %u: %s\n", progname, cap, strerror(errno));
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    for (i = 3; i < argc; i++) {
        if (strcmp(argv[i], "-v") == 0) {
            ...
            if (i == argc) {
                fprintf(stderr, "%s: missing argument to option '-t'\n", progname);
                exit(1);
            }
            type = argv[i];
            ...
            if (!type[0]) {
                fprintf(stderr, "%s: empty type given as argument to option '-t'\n", progname);
                exit(1);
            }
        }
            ...
            while (opt) {
                int         j;
                int         ignore        = 0;
                const char *ignore_opts[] = {"", "user", "nofail", "nouser", "users", "auto", "noauto", "_netdev", NULL};
                if (strncmp(opt, "setuid=", 7) == 0) {
                    setuid_name = xstrdup(opt + 7);
                    ignore      = 1;
                }
                else if (strcmp(opt, "drop_privileges") == 0) {
                    pass_fuse_fd = 1;
#if defined(CAP_TO_MASK)
                    drop_privileges = 1;
#endif  // CAP_TO_MASK
...
#if defined(CAP_TO_MASK)
    if (drop_privileges) {
        uint64_t required_caps = CAP_TO_MASK(CAP_SETPCAP) | CAP_TO_MASK(CAP_SYS_ADMIN);
        if ((get_capabilities() & required_caps) != required_caps) {
            fprintf(stderr,
                    "%s: drop_privileges was requested, which launches the FUSE file system fully unprivileged. In order to do so "
                    "%s must be run with privileges, please invoke with CAP_SYS_ADMIN and CAP_SETPCAP (e.g. as root).\n",
                    progname, progname);
            exit(1);
        }
    }
