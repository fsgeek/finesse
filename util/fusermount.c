...
static int add_mount(const char *source, const char *mnt, const char *type, const char *opts)
{
...
static int may_unmount(const char *mnt, int quiet)
{
    ...
    while ((entp = getmntent(fp)) != NULL) {
        if (!found && strcmp(entp->mnt_dir, mnt) == 0 &&
            (strcmp(entp->mnt_type, "fuse") == 0 || strcmp(entp->mnt_type, "fuseblk") == 0 ||
             strncmp(entp->mnt_type, "fuse.", 5) == 0 || strncmp(entp->mnt_type, "fuseblk.", 8) == 0)) {
            char *p = strstr(entp->mnt_opts, "user=");
            if (p && (p == entp->mnt_opts || *(p - 1) == ',') && strcmp(p + 5, user) == 0) {
                found = 1;
                break;
            }
            /* /etc/mtab is a link pointing to
               /proc/mounts: */
            else if ((p = strstr(entp->mnt_opts, "user_id=")) && (p == entp->mnt_opts || *(p - 1) == ',') &&
                     strncmp(p + 8, uidstr, uidlen) == 0 && (*(p + 8 + uidlen) == ',' || *(p + 8 + uidlen) == '\0')) {
                found = 1;
                break;
            }
        }
...
static int check_is_mount_child(void *p)
{
    ...
    if (fp == NULL) {
        fprintf(stderr, "%s: failed to open %s: %s\n", progname, procmounts, strerror(errno));
        return 1;
    }
    ...
    while ((entp = getmntent(fp)) != NULL) {
        ...
        if (entp->mnt_dir[0] == '/' && strcmp(entp->mnt_dir + 1, last) == 0 && (!type || strcmp(entp->mnt_type, type) == 0)) {
            found = 1;
            break;
        }
    }
...
#ifdef __ia64__
    extern int __clone2(int (*fn)(void *), void *child_stack_base, size_t stack_size, int flags, void *arg, pid_t *ptid, void *tls,
                        pid_t *ctid);

    return __clone2(check_is_mount_child, stack, sizeof(buf) / 2, CLONE_NEWNS, a, NULL, NULL, NULL);
#else
...
static void read_conf(void)
{
    ...
    if (fp != NULL) {
        ...
        bool fatal = (errno != EACCES && errno != ELOOP && errno != ENAMETOOLONG && errno != ENOTDIR && errno != EOVERFLOW);
...
static struct mount_flags mount_flags[] = {{"rw", MS_RDONLY, 0, 1},         {"ro", MS_RDONLY, 1, 1},
                                           {"suid", MS_NOSUID, 0, 0},       {"nosuid", MS_NOSUID, 1, 1},
                                           {"dev", MS_NODEV, 0, 0},         {"nodev", MS_NODEV, 1, 1},
                                           {"exec", MS_NOEXEC, 0, 1},       {"noexec", MS_NOEXEC, 1, 1},
                                           {"async", MS_SYNCHRONOUS, 0, 1}, {"sync", MS_SYNCHRONOUS, 1, 1},
                                           {"atime", MS_NOATIME, 0, 1},     {"noatime", MS_NOATIME, 1, 1},
                                           {"dirsync", MS_DIRSYNC, 1, 1},   {NULL, 0, 0, 0}};
...
static int get_mnt_opts(int flags, char *opts, char **mnt_optsp)
{
    ...
    for (i = 0; mount_flags[i].opt != NULL; i++) {
        if (mount_flags[i].on && (flags & mount_flags[i].flag) && add_option(mnt_optsp, mount_flags[i].opt, 0) == -1)
            return -1;
    }
...
static int get_string_opt(const char *s, unsigned len, const char *opt, char **val)
{
    int      i;
 ...
 */
static int mount_notrunc(const char *source, const char *target, const char *filesystemtype, unsigned long mountflags,
                         const char *data)
{
...
static int do_mount(const char *mnt, const char **typep, mode_t rootmode, int fd, const char *opts, const char *dev, char **sourcep,
                    char **mnt_optsp)
{
    int         res;
    ...
    for (s = opts, d = optbuf; *s;) {
        ...
        bool        escape_ok   = begins_with(s, fsname_str) || begins_with(s, subtype_str);
        for (len = 0; s[len]; len++) {
        ...
        if (begins_with(s, fsname_str)) {
        ...
        else if (!begins_with(s, "fd=") && !begins_with(s, "rootmode=") && !begins_with(s, "user_id=") &&
            ...
            if (opt_eq(s, len, "large_read")) {
                ...
                unsigned       kmaj, kmin;
                res = uname(&utsname);
                if (res == 0 && sscanf(utsname.release, "%u.%u", &kmaj, &kmin) == 2 && (kmaj > 2 || (kmaj == 2 && kmin > 4))) {
                    fprintf(stderr, "%s: note: 'large_read' mount option is deprecated for %i.%i kernels\n", progname, kmaj, kmin);
                    skip_option = 1;
                }
            }
            if (getuid() != 0 && !user_allow_other && (opt_eq(s, len, "allow_other") || opt_eq(s, len, "allow_root"))) {
            ...
            if (!skip_option) {
                ...
                else if (opt_eq(s, len, "default_permissions") || opt_eq(s, len, "allow_other") || begins_with(s, "max_read=") ||
    ...
    *d  = '\0';
    ...
    sprintf(d, "fd=%i,rootmode=%o,user_id=%u,group_id=%u", fd, rootmode, getuid(), getgid());

    source = malloc((fsname ? strlen(fsname) : 0) + (subtype ? strlen(subtype) : 0) + strlen(dev) + 32);
    ...
    if (res == -1 && errno == EINVAL) {
        ...
        sprintf(d, "fd=%i,rootmode=%o,user_id=%u", fd, rootmode, getuid());
        res = mount_notrunc(source, mnt, type, flags, optbuf);
    }
    if (res == -1) {
...
static int check_perm(const char **mntp, struct stat *stbuf, int *mountpoint_fd)
{
    ...
    if (S_ISDIR(stbuf->st_mode)) {
        ...
        *mntp = procfile;
        ...
        fprintf(stderr, "%s: mountpoint %s is not a directory or a regular file\n", progname, mnt);
        return -1;
    }
...
static int try_open(const char *dev, char **devp, int silent)
{
    ...
    else if (errno == ENODEV || errno == ENOENT) /* check for ENOENT too, for the udev case */
        return -2;
    else if (!silent) {
...
static int mount_fuse(const char *mnt, const char *opts, const char **type)
{
    ...
    restore_privs();
    if (res != -1)
        res = do_mount(real_mnt, type, stbuf.st_mode & S_IFMT, fd, opts, dev, &source, &mnt_opts);
...
int main(int argc, char *argv[])
{
    ...
    static const struct option long_opts[] = {{"unmount", no_argument, NULL, 'u'}, {"lazy", no_argument, NULL, 'z'},
