...
static MunitResult test_finess_started(const MunitParameter params[] __notused, void *prv __notused)
{
...
static MunitResult test_open_dir(const MunitParameter params[] __notused, void *prv __notused)
{
    int         fd;
    const char *prefix;
    const char *pathname;
...
static MunitResult test_open_existing_files(const MunitParameter params[] __notused, void *prv __notused)
{
    int         fd;
    const char *prefix;
    int         index;
...
static MunitResult test_open_nonexistant_files(const MunitParameter params[] __notused, void *prv __notused)
{
    int           fd;
    const char *  prefix;
    const char *  dir;
...
static MunitResult test_fstatfs(const MunitParameter params[] __notused, void *prv __notused)
{
    int            fd;
    int            status;
    const char *   prefix;
...
static MunitResult test_stat(const MunitParameter params[] __notused, void *prv __notused)
{
    int         fd;
    const char *prefix;
    int         index;
    ...
    while (NULL != test_files[index]) {
        ...
        if (finesse_enabled) {
            status = finesse_stat(scratch, &statbuf);
        }
        else {
            status = stat(scratch, &statbuf);
        }
        munit_assert(0 == status);
        index++;
    }
