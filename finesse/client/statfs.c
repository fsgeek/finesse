...
//}
//
//int finesse_statfs(fuse_ino_t nodeid)
//{
//    int status;
//
//    finesse_init();
//
//    status = fin_statfs_call(nodeid);
//
//    if (0 > status) {
//        status = fin_statfs(nodeid);
//    }
//
//    return status;
//}
//
//static int fin_statfs_call(fuse_ino_t nodeid)
//{
//    int status;
//    uint64_t req_id;
//
//    status = FinesseSendStatfsRequest(finesse_client_handle, nodeid, &req_id);
//    while (0 == status) {
//        status = FinesseGetStatfsResponse(finesse_client_handle, req_id);
//        break;
//    }
//
//    return status;
...
int FinesseSendFstatfsResponse(finesse_server_handle_t FinesseServerHandle, uuid_t *ClientUuid, uint64_t RequestId, struct statvfs *buf, int64_t Result)
{
	...
	rsp.fstatfsrsp->statfsstruc->f_bsize = buf->f_bsize;
	rsp.fstatfsrsp->statfsstruc->f_blocks = buf->f_blocks;
	rsp.fstatfsrsp->statfsstruc->f_bfree = buf->f_bfree;
	rsp.fstatfsrsp->statfsstruc->f_bavail = buf->f_bavail;
	rsp.fstatfsrsp->statfsstruc->f_files = buf->f_files;
	rsp.fstatfsrsp->statfsstruc->f_ffree = buf->f_ffree;
	rsp.fstatfsrsp->statfsstruc->f_fsid = buf->f_fsid;
	rsp.fstatfsrsp->statfsstruc->f_frsize = buf->f_frsize;
	rsp.fstatfsrsp->statfsstruc->f_flag = buf->f_flag;
	rsp.fstatfsrsp->statfsstruc->f_namemax = buf->f_namemax;
	rsp.fstatfsrsp->statfsstruc->f_favail = buf->f_favail;	
