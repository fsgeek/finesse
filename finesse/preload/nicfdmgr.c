...
/*
 * Note: at the present time, the nicfs_file_state_t structure is **Not** reference counted.
 *       Instead, it relies upon the open/close management logic to know when it is time
 *       to delete the state.
 */

nicfs_file_state_t *nicfs_create_file_state(int fd, nicfs_key_t *key, const char *pathname)
{
    size_t pathlen = strlen(pathname) + sizeof('\0');
    size_t size = (sizeof(nicfs_file_state_t) + pathlen + 0x7) & ~0x7;
...
nicfs_file_state_t *nicfs_lookup_file_state(int fd)
{
    nicfs_file_state_t *file_state;
    int status;
...
void nicfs_update_offset(nicfs_file_state_t *file_state, size_t offset)
{
    file_state->current_offset = offset;
}

void nicfs_delete_file_state(nicfs_file_state_t *file_state)
{
    int fd;
    int status; 
...
int nicfs_init_file_state_mgr(void)
{
...
void nicfs_terminate_file_state_mgr(void)
{
...
//
void nicfs_insert_new_fd(int fd, const char *path)
{
    (void) fd;
    (void) path;
#if 0
    nicfs_key_t *nicfs_key = malloc(sizeof(nicfs_key_t));

    (void) fd;
    (void) path;

    if (NULL == fd_lookup_table) {
        nicfs_init_file_state_mgr();
