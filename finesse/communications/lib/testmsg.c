...
int FinesseSendTestRequest(finesse_client_handle_t FinesseClientHandle, uint64_t *RequestId)
{
    ...
    assert(NULL != message);
    message->OperationType = FINESSE_FUSE_REQ_TEST;
...
int FinesseSendTestResponse(finesse_server_handle_t FinesseServerHandle, uuid_t *ClientUuid, uint64_t RequestId, int64_t Result)
{
    ...
    assert(0 != RequestId);
    assert(FINESSE_FUSE_MSG_REQUEST == message->OperationType);
    
    message->Result = Result;
    message->OperationType = FINESSE_FUSE_MSG_RESPONSE;

    ffm = (finesse_fuse_msg *)message->Data;
    memset(ffm, 0, sizeof(finesse_fuse_msg)); // not necessary for production
    ffm->Version = FINESSE_FUSE_VERSION;
