    ...
    while (NULL != inode) {
        ...
        while ((used + FUSE_NAME_OFFSET_DIRENTPLUS) < size) {
            ...
            if (bitbucket_debug_readdirplus) {
                fuse_log(FUSE_LOG_DEBUG, "Finesse (%s): added entry for %s with inode 0x%lx (%lu), mode = %o\n", __func__,
                         dirEntry->Name, dirEntry->Inode->Attributes.st_ino, dirEntry->Inode->Attributes.st_ino,
