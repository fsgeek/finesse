...
static MunitResult test_insert(const MunitParameter params[] __notused, void *prv __notused)
{
...
static MunitResult test_lookup(const MunitParameter params[] __notused, void *prv __notused)
{
    bitbucket_inode_t *      rootdir = NULL;
    int                      status  = 0;
    uint64_t                 refcount;
...
static MunitResult test_remove(const MunitParameter params[] __notused, void *prv __notused)
{
    bitbucket_inode_t *      rootdir = NULL;
    int                      status  = 0;
    uint64_t                 refcount;
    ...
    status = BitbucketRemoveExtendedAttribute(testfile, testattrname);
    munit_assert(0 == status);
    status = BitbucketRemoveExtendedAttribute(testfile, testattrname);
    munit_assert(0 != status);  // shouldn't work the second time
