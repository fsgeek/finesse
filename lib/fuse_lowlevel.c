...
/* `buf` is allowed to be empty so that the proper size may be
   allocated by the caller */
size_t fuse_add_direntry_plus(fuse_req_t req, char *buf, size_t bufsize, const char *name, const struct fuse_entry_param *e,
                              off_t off)
{
    (void)req;
    size_t namelen;
...
static int fuse_send_data_iov_fallback(struct fuse_session *se, struct fuse_chan *ch, struct iovec *iov, int iov_count,
                                       struct fuse_bufvec *buf, size_t len)
{
    ...
    if (buf->count == 1 && buf->idx == 0 && buf->off == 0 && !(buf->buf[0].flags & FUSE_BUF_IS_FD)) {
...
static int fuse_pipe(int fds[2])
{
    ...
    if (fcntl(fds[0], F_SETFL, O_NONBLOCK) == -1 || fcntl(fds[1], F_SETFL, O_NONBLOCK) == -1 ||
        fcntl(fds[0], F_SETFD, FD_CLOEXEC) == -1 || fcntl(fds[1], F_SETFD, FD_CLOEXEC) == -1) {
        close(fds[0]);
        close(fds[1]);
        rv = -1;
    }
...
static struct fuse_ioctl_iovec *fuse_ioctl_iovec_copy(const struct iovec *iov, size_t count)
{
...
int fuse_reply_ioctl_retry(fuse_req_t req, const struct iovec *in_iov, size_t in_count, const struct iovec *out_iov,
                           size_t out_count)
{
...
static void do_batch_forget(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
    ...
    if (req->se->op.forget_multi) {
        req->se->op.forget_multi(req, arg->count, (struct fuse_forget_data *)param);
    }
...
static void do_setattr(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
    ...
    if (req->se->op.setattr) {
        ...
        arg->valid &= FUSE_SET_ATTR_MODE | FUSE_SET_ATTR_UID | FUSE_SET_ATTR_GID | FUSE_SET_ATTR_SIZE | FUSE_SET_ATTR_ATIME |
...
static void do_rename2(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
    ...
    if (req->se->op.rename)
        req->se->op.rename(req, nodeid, oldname, arg->newdir, newname, arg->flags);
    else
        fuse_reply_err(req, ENOSYS);
}
...
static void do_write_buf(fuse_req_t req, fuse_ino_t nodeid, const void *inarg, const struct fuse_buf *ibuf)
{
    struct fuse_session *se   = req->se;
    ...
    if (se->conn.proto_minor < 9) {
        ...
        bufv.buf[0].size -= sizeof(struct fuse_in_header) + FUSE_COMPAT_WRITE_IN_SIZE;
...
static void convert_fuse_file_lock(struct fuse_file_lock *fl, struct flock *flock)
{
...
static void do_setlk_common(fuse_req_t req, fuse_ino_t nodeid, const void *inarg, int sleep)
{
    struct fuse_lk_in *   arg = (struct fuse_lk_in *)inarg;
    struct fuse_file_info fi;
    struct flock          flock;
...
static int find_interrupted(struct fuse_session *se, struct fuse_req *req)
{
    ...
    for (curr = se->interrupts.next; curr != &se->interrupts; curr = curr->next) {
...
static struct fuse_req *check_interrupt(struct fuse_session *se, struct fuse_req *req)
{
    struct fuse_req *curr;
...
static void do_ioctl(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
    ...
    if (sizeof(void *) == 4 && req->se->conn.proto_minor >= 16 && !(flags & FUSE_IOCTL_32BIT)) {
        req->ioctl_64bit = 1;
    }

    if (req->se->op.ioctl)
        req->se->op.ioctl(req, nodeid, arg->cmd, (void *)(uintptr_t)arg->arg, &fi, flags, in_buf, arg->in_size, arg->out_size);
    else
        fuse_reply_err(req, ENOSYS);
}
...
static void do_copy_file_range(fuse_req_t req, fuse_ino_t nodeid_in, const void *inarg)
{
    ...
    if (req->se->op.copy_file_range)
        req->se->op.copy_file_range(req, nodeid_in, arg->off_in, &fi_in, arg->nodeid_out, arg->off_out, &fi_out, arg->len,
                                    arg->flags);
    else
        fuse_reply_err(req, ENOSYS);
}
...
static void list_add_nreq(struct fuse_notify_req *nreq, struct fuse_notify_req *next)
{
...
static void do_notify_reply(fuse_req_t req, fuse_ino_t nodeid, const void *inarg, const struct fuse_buf *buf)
{
    struct fuse_session *   se = req->se;
...
static int send_notify_iov(struct fuse_session *se, int notify_code, struct iovec *iov, int count)
{
    struct fuse_out_header out;
...
int fuse_lowlevel_notify_store(struct fuse_session *se, fuse_ino_t ino, off_t offset, struct fuse_bufvec *bufv,
...
static void fuse_ll_retrieve_reply(struct fuse_notify_req *nreq, fuse_req_t req, fuse_ino_t ino, const void *inarg,
...
static int fuse_ll_copy_from_pipe(struct fuse_bufvec *dst, struct fuse_bufvec *src)
{
...
{
    const size_t           write_header_size = sizeof(struct fuse_in_header) + sizeof(struct fuse_write_in);
    ...
    if (se->debug) {
        fuse_log(FUSE_LOG_DEBUG, "unique: %llu, opcode: %s (%i), nodeid: %llu, insize: %zu, pid: %u\n",
                 (unsigned long long)in->unique, opname((enum fuse_opcode)in->opcode), in->opcode, (unsigned long long)in->nodeid,
                 buf->size, in->pid);
    }
    ...
    if (se->deny_others && in->uid != se->owner && in->uid != 0 && in->opcode != FUSE_INIT && in->opcode != FUSE_READ &&
        in->opcode != FUSE_WRITE && in->opcode != FUSE_FSYNC && in->opcode != FUSE_RELEASE && in->opcode != FUSE_READDIR &&
        in->opcode != FUSE_FSYNCDIR && in->opcode != FUSE_RELEASEDIR && in->opcode != FUSE_NOTIFY_REPLY &&
    ...
    if ((buf->flags & FUSE_BUF_IS_FD) && write_header_size < buf->size && (in->opcode != FUSE_WRITE || !se->op.write_buf) &&
...
static const struct fuse_opt fuse_ll_opts[] = {LL_OPTION("debug", debug, 1), LL_OPTION("-d", debug, 1),
...
int fuse_session_receive_buf_int(struct fuse_session *se, struct fuse_buf *buf, struct fuse_chan *ch)
{
     ...
     */
    if (res < sizeof(struct fuse_in_header) + sizeof(struct fuse_write_in) + pagesize) {
        ...
        if (!buf->mem) {
            ...
            if (!buf->mem) {
                fuse_log(FUSE_LOG_ERR, "fuse: failed to allocate read buffer\n");
                return -ENOMEM;
            }
        }
        buf->size  = se->bufsize;
